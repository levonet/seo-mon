...
module.exports = class Seomon {
        ...
        return parser.browser
            ...
            .then(() => {
                if (!parser.data.length) {
                    log.error(`NO DATA in query '${query.text}' [${this._browserName}]`);
    ...
    /**
     * Ищет в поисковой выдаче на открытой странице совпадения с хостами из `monitoring`.
     * Ссылки и позиция найденного сохраняется в `parser.data`.
     * @param parser        - Данные в рамках текущего запроса
     * @param {number} page - Номер страницы
     * @returns {Promise<Array>} Массив с тем, что возвращает `parseSerpItemWithRetry()`
     */
    ...
    /**
     * Обертка, которая позволяет перезапустить перезапустить предустановленное число раз
    ...
    parseSerpItemWithRetry(parser, elemId, _retry) {
        if (_retry === undefined) {
            ...
            log.info(`Item '${elemId}' parsing will be retried. Retries left: ${_retry}`);
        }

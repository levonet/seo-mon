...
module.exports = class Seomon {
    ...
    run() {
        if (this._queryListFile === undefined) {
            console.error('Query list file does not specify');
            process.exit(100);
        }
        ...
        this._serialAsyncMap(utl.shuffle(querys), this.parseSerpWithRetry)
            .then(() => {
                const message = [
                    'Total: ' + stat.count,
                    'Failed: ' + stat.errors,
                    'Without data: ' + stat.nodata,
                    'Query retries: ' + stat.queryRetries,
                    'Item parsing retries: ' + stat.itemRetries,
                ];

                console.log('\n' + message.join('  ') + '\n');

                if (stat.count === 0 ||
                    stat.count === stat.nodata ||
                    stat.errors) {
    ...
    parseSerp(query) {
        ...
        console.log(`${stat.count} '${url.toString()}&q=${query.text}' [${this._browserName}]`);
        ...
        return parser.browser
            ...
            .then((currentUrl) => {
                ...
                return this.parseSerpPage(parser)
                    .then((page) => {
                        log.info(`${stat.count}:${page + 1} '${url.toString()}&q=${query.text}' [${this._browserName}]`);
                    }, (err) => {
            ...
            .then(() => {
                if (!parser.data.length) {
                    return parser.browser
                        ...
                        .then(() => {
                            ++stat.nodata;
                            log.info(`Warning: nothing found in query '${query.text}' [${this._browserName}]`);
                        });
                }
